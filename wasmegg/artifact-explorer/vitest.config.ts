...
export default defineConfig({
  ...
  // per key, which means parsing and re-emitting the whole thing as JavaScript source, at ~24s per worker,
  // paid on every run. `stringify` emits a single `JSON.parse('…')` instead, which the engine parses far faster.
